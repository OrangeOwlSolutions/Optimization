...
/***************************/
//extern "C" __host__ __device__ float transformation(const float * __restrict__, const int);
//__device__ float trans(const float * __restrict__ x, const int i) { return transformation(x, i); };
// __host__ __device__ float trans(const float * __restrict__ x, const int i) { transformation(x, i); };
template <class T>
//__host__ __device__ T transformation(const T * __restrict__ x, const int i) { return ((T)100 * (x[i+1] - x[i] * x[i]) * (x[i+1] - x[i] * x[i]) + (x[i] - (T)1) * (x[i] - (T)1)) ; }
...
//__host__ __device__ __forceinline__ T transformation(const T * __restrict__ x, const int i) { return x[i]; }
//__host__ __device__ __forceinline__ T transformation(const T * __restrict__ x, const int i) { return log(x[i]) * x[i]; }

//__host__ __device__ __forceinline__ float transformation(const float * __restrict__ x, const int i) { return (100.f * (x[i+1] - x[i] * x[i]) * (x[i+1] - x[i] * x[i]) + (x[i] - 1.f) * (x[i] - 1.f)) ; }
